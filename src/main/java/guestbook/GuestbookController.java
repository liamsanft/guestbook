...
class GuestbookController {
    ...
    /**
     * Increases the like count of a {@link GuestbookEntry}.
     *
     * @param id the unique id of the {@link GuestbookEntry}, where the like count should be increased
     * @return a 200 response with a browser refresh
     */
    @HxRequest
    @PostMapping(path = "/guestbook/increase-likes/{id}")
    HtmxResponse addLike(@PathVariable Long id) {
        Optional<GuestbookEntry> entry = guestbook.findById(id);

        if (entry.isPresent()) {
            entry.get().increaseLikes();
